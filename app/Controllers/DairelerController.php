...
public function topluEkle()
{
    helper(['form', 'url']);
    $model = new DairelerModel();

    $blockCount = (int)$this->request->getVar('txtBlokSayisi');

    for ($i = 1; $i <= $blockCount; $i++) {
        $blokAdi = $this->request->getVar("selectBlok{$i}");
        $daireSayisi = (int)$this->request->getVar("txtDaireSayisi{$i}");

        for ($j = 1; $j <= $daireSayisi; $j++) {
            $data = [
                'blok_adi' => $blokAdi,
                'daire_no' => $j,
            ];
    ...
    return redirect()->to(site_url('daireler'));
}
}